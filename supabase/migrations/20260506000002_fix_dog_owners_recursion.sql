-- Drop the recursive policy that causes infinite recursion
drop policy if exists "Dog members can view all family members" on dog_owners;
drop policy if exists "Members can read their own dog_owners row" on dog_owners;

-- Security-definer function to check if caller is a member of a dog
-- without triggering RLS recursion
create or replace function public.is_dog_member(p_dog_id uuid)
returns boolean
language sql
security definer
stable
set search_path = public
as $$
  select exists (
    select 1 from public.dog_owners
    where dog_id = p_dog_id
      and user_id = auth.uid()
  )
  or
  exists (
    select 1 from public.dogs
    where id = p_dog_id
      and owner_id = auth.uid()
  );
$$;

-- Allow any member (owner or invited) to see ALL dog_owners rows for that dog
create policy "Dog members can view all family members"
  on public.dog_owners for select using (
    public.is_dog_member(dog_id)
  );
